...
class Classification(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        inputs.ndata["x"] = inputs.ndata["x"].permute(0, 3, 1, 2) # TODO why permute?
        inputs.edata["x"] = inputs.edata["x"].permute(0, 2, 1)    # TODO why different permutations?
...
class Regression(pl.LightningModule):
    ...
    def forward(self, batched_graph):
        y_pred = self.model(batched_graph)
        return y_pred
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = F.l1_loss(pred, labels)
        self.log("test_loss", loss, on_step=False, on_epoch=True, sync_dist=True)
        print(f"test step for batch {batch_idx}:")
...
class UVNetRegressor(nn.Module):
    ...
    def __init__(
        ...
        self.clf = _NonLinearRegressor(graph_emb_dim, dropout)
    ...
    def forward(self, batched_graph):
        ...
        hidden_srf_feat = self.surf_encoder(input_srf_feat)
        # Message pass and compute per-face(node) and global embeddings
        # Per-face embeddings are ignored during solid regression
        _, graph_emb = self.graph_encoder(
            batched_graph, hidden_srf_feat, hidden_crv_feat
        )
...
class _NonLinearRegressor(nn.Module):
